...
\Omnifold~\cite{Andreassen:2019cjw} is the unbinned, classifier-based implementation of the EM algorithm for unfolding introduced in \Cref{sec:unfolding-integral-equation}.
...
Comparing \Cref{eq:omnifold-estep} with the IBU E-step in \Cref{eq:ibu2}, the ratio $w_n(m)$ plays the same role as $\Pr(d_j \mid t_i)$ in the binned case, with the sum over bins replaced by an integral over the continuous detector-level space.

The M-step computes the corresponding truth-level ratio
\begin{align}
\label{eq:omnifold-mstep}
\nu_n(t) = \frac{\displaystyle\int w_n(m)\, p(m \mid t)\, dm}{\nu^{\mathrm{of}}_{n-1}(t)\, p_{\mathrm{gen}}(t)},
\end{align}
which is the expected value of the detector-level reweighting function $w_n(m)$ under $p(m \mid t)$, normalized by the current prior.
This expectation again requires the truth-to-detector mapping supplied by the simulation.
The numerator is the continuous analog of the sum over $j$ in the IBU M-step of \Cref{eq:ibu3}, confirming that \Omnifold is the zero-bin-width limit of IBU.
...
This procedure has been shown to converge to the maximum likelihood solution of the folding equation~\cite{Andreassen:2019cjw}; convergence can also be understood as the zero-bin-width limit of the analogous result for IBU~\cite{Shepp:1982}.
...
\begin{figure}[tb]
    ...
    Step~1 trains at detector level to reweight MC simulation to data; Step~2 trains at truth level to push those weights back to the truth-level simulation.
...
The detector-level weights $w_n(m)$ are propagated to the truth-level simulation by assigning each truth-level MC event the weight of its paired detector-level counterpart.
...
The events forming signal and background are identical; only their weights differ.
...
If more iterations are required, the updated weights are pushed back to the detector-level simulation and Steps 1 and 2 repeat.
...
The output of the unfolding is a reweighted MC simulation, not a set of generated samples.
...
The method handles variable-dimensional distributions naturally.
...
Crucially, training a set-processing classifier is substantially easier than training a set-processing generative model, which is the alternative in the generative EM approach.
