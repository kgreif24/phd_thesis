...
In this context, unfolding can be posed as a concrete inference problem: given a detector-level histogram and a sample of simulated events for which both the truth-level and detector-level configurations are known, infer the truth-level histogram.
The simulated events are used to construct a \textit{response matrix} that encodes how events migrate between bins at truth level and bins at detector level.
...
Equivalently, the maximum likelihood solution can be highly unstable: small changes in the observed data lead to large and unphysical oscillations in the inferred spectrum.
Most binned unfolding methods address this by deliberately injecting a small bias into the solution in order to reduce the variance, a procedure known as \textit{regularization}.
The residual bias introduced by regularization must then be estimated and propagated as a systematic uncertainty on the final result.

\subsection{Iterative Bayesian Unfolding}
\label{sec:ibu}

The most widely used binned unfolding method in high-energy physics is \textit{Iterative Bayesian Unfolding} (IBU)~\cite{DAgostini:1994fjx}, also known as Lucy-Richardson deconvolution in other fields~\cite{Lucy:1974yx, Richardson:1972}.
IBU is an expectation-maximization (EM) algorithm~\cite{Shepp:1982}.
For clarity, the derivation below assumes $\textbf{b}=\textbf{0}$ and $\textbf{f}=\textbf{c}=\textbf{1}$, so that the folding equation reduces to $\textbf{R}\cdot\textbf{t} = \textbf{d}$.
...
The connection to the EM algorithm is explicit in the structure of these equations.
The \textit{expectation step} (E-step) is the computation in \Cref{eq:ibu2}: holding the current prior $\Pr^{(n)}(t_i)$ fixed, one calculates the posterior probability $\Pr(t_i | d_j)$ that a detector-level event in bin $j$ originated from truth-level bin $i$, using Bayes' theorem with the response matrix elements $R_{ji} = \Pr(d_j | t_i)$ as the likelihood.
The \textit{maximization step} (M-step) is the sum in \Cref{eq:ibu1} and \Cref{eq:ibu3}: the posterior weights are used to redistribute the observed detector-level counts $d_j$ back to truth-level bins, yielding the updated estimate $t_i^{(n)}$, which then serves as the prior for the next iteration.
Because the number of iterations controls how far the estimate departs from the simulation prior, stopping the iteration early acts as the regularization mechanism in IBU: few iterations keep the result close to the prior (high bias, low variance), while many iterations approach the maximum likelihood solution (low bias, high variance).
...
IBU is the only binned method directly relevant to this thesis, so other methods are not discussed in detail.
Binned unfolding can be extended beyond one-dimensional histograms by unrolling multi-dimensional histograms and response matrices into one-dimensional vectors and matrices.
This approach has been applied to measurements in up to three dimensions~\cite{CMS:2024mlf}, but it rapidly becomes impractical as the number of dimensions grows.
The number of bins scales exponentially with the number of observables, making it impossible to populate the response matrix with sufficient simulation.
This fundamental limitation, known as the curse of dimensionality, is one of the primary motivations for unbinned unfolding methods, which are the subject of the next section.
