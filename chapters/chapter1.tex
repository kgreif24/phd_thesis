...
\begin{enumerate}
    ...
    \item{
        ...
        None of the Standard Model particles could account for this \textit{dark matter}.
...
The particle physics community adopted deep learning very soon after AlexNet showed promise in computer vision.
The first papers proposing deep learning based jet taggers appeared in 2014~\cite{Cogan:2014oua,Baldi:2014kfa}, only two years after AlexNet.
...
RL has yet to find widespread application in particle physics, but its broader trajectory is relevant to the discussion of agentic AI in the conclusion.
...
The introduction of a reinforcement learning based post-training~\cite{Ouyang:2022instructgpt} to LLMs transformed them from autocomplete engines into conversational assistants capable of writing code, reasoning through multi-step problems, and passing professional licensing examinations such as the bar exam.
...
Chapter~\ref{ch:ml} introduces the deep learning methods that underpin the research chapters that follow.
...
Two AI-based methods are then described in detail: the Variational Latent Diffusion (VLD) model which I developed with collaborators, and \Omnifold~\cite{Andreassen:2019cjw}.
...
After describing the methodology, the chapter presents results across four physics use cases that illustrate the unique power of unbinned and high-dimensional unfolding for extracting physics from LHC data.
