...
\begin{figure}[ht]
  ...
  \caption{The two-point energy correlator measured in CMS 2011 open data for high \pt jets, showing distinct free-hadron, transition, and quark/gluon regimes as a function of the angular scale $R_L$ (called $z$ in the text). Figure adapted from Ref.~\cite{Komiske:2022enw} under a CC~BY~4.0 license.}
...
% Additionally the NNID estimator at low values of $i$, which probes the short distances in the dataset, is very sensitive to negative weights, as shown in \Cref{fig:zjets-results-nnid-neg-weights}.
% The figure shows that the \mgpy MC sample, which has some fraction of negative event weights, produces NNID estimates with unphysical noise at small median EMD scales, corresponding to low values of the unphysical index $i$.
% This behavior is absent in the all-positive reweighted \mgpy sample and the detector level data.
% The negative weights cause the instability because some jets add negative mass to the nearest neighbor calculation.
% For this reason the measurements in \Cref{ch:zjets} are not compared to the generator predictions at very low median EMD scales.

% \begin{figure}[tb]
%     \centering
%     \includegraphics[width=0.65\linewidth, alt={Comparison of nearest-neighbor intrinsic dimension estimates with and without negative weights, showing that the estimator is sensitive to negative weights at low neighbor index values.}]{figures/ch2/nnid_neg_weight_comparison.pdf}
%     \caption{Comparison of NNID estimates provided by three datasets, the raw \mgpy generator predictions with a fraction of negative event weights, an all positive reweighted version of the \mgpy sample, and detector-level data which has event weights of $1.0$ by construction. The NNID estimator on the MC sample with a fraction of negative weights has unphysical noise at small median EMD. This behavior is absent in the all-positive reweighted MC sample and the detector level data.}
%     \label{fig:zjets-results-nnid-neg-weights}
% \end{figure}
