...
In practice this property of MLPs is of limited use because it guarantess only that a solution exists, not that it can be found by any of the gradient-based optimization strategies discussed below.
...
Minimizing the BCE loss is equivalent to maximizing the log-likelihood of the data given their labels are described by a Bernoulli random variable.
...
In \Cref{eq:gd}, the loss is computed over the full dataset before gradients are computed.
...
A trained may not perform well on unseen data if it has ``memorized'' the specific data points rather than learning the underlying function.
...
A simple regularization strategy is regularization, in which training is terminated when the validation loss stops decreasing, preventing the network from entering the high variance overfitting regime.
...
At inference time, all activations are activated.
...
This is achieved by \textit{weight sharing}, in which a set of weights and biases, often called a \textit{filter}, is scanned across the inputs rather than applying a separate set of weightsper position.
...
The space output channels is equal to the number of filters $F = C_{\text{out}}$.
...
The filters of a given convolutional layer are sometimes applied with a \textit{stride} $s > 1$, meaning they advances by $s$ positions at each step.
...
Another important operation in CNNs is \textit{pooling}, in which an aggregation operation like maximum (\textit{max pooling}) or averae (\textit{average pooling}) is applied across a spatial region.
...
ResNet was the dominant image classification architecture for several years after in was introduced in 2015.
...
The relative features $\mathbf{h}_j - \mathbf{h}_i$ capture pairwise distances between inputs and are empirically found to be very useful in HEP, for example because the difference between two particle's four momenta is physically meaningful.
...
The better expresivity and better scaling properties of full attention, in which the strenght of two inputs connection is allowed to be a learned feature, has led attention-based networks to largely supplant GNNs in HEP applications.
...
In early versions of attention the decoder of an RNN, whose job is to generate the next output token given the networks current internal representation, could selectively weight all positions in the input sequence according to their relevance to the current decoding step.
...
The $(i,j)$-th entry of this matrix can be interpreted as setting how much the input at position $i$ (query) should effect the item at position $j$ (key).
...
It captures relationships between inputs, but doesn't allow the element-wise processing of representations.
