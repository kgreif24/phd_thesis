...
Architecture design does matter, and though there are some theoretical motivations for why this might be the case~\cite{jacot2018neural} there is not a rigorous theory explaining why some networks optimize better than others.
...
The BCE loss has a well-understood probabilistic interpretation.
...
Convolutional neural networks (CNNs) address this by encoding \textit{translational symmetry} into the network architecture, meaning that a feature at one spatial location should not affect the network output if shifted to another location.
